...
class TwoPhase:
    ...
    def add_rate_well(
        self, injection_rate_water, injection_rate_oil, cell_index, well_name
    ):
    ...
    def initialize_system(self):
        ...
        print("building m")
        [self.m_x_coo_data, m_x_coo_row, m_x_coo_col] = self.mfd.build_m(
            save_update_info=True
        )
        ...
        self.div = sparse.coo_matrix(
            (
                np.array(div_data),
                (np.add(np.array(div_row), -self.mfd.flux_dof), np.array(div_col)),
            )
        )
        ...
        self.lhs_coo = sparse.coo_matrix(
            (np.array(lhs_data), (np.array(lhs_row), np.array(lhs_col)))
        )
        ...
        self.newton_solution = np.zeros(
            self.mesh.get_number_of_cells() + self.mfd.flux_dof
        )
        ...
        if self.solver == 1:
            ...
            self.m_coo = sparse.coo_matrix((m_info[0], (m_info[1], m_info[2])))

            m_csr = self.m_coo.tocsr()

            self.m_petsc = PETSc.Mat()
            self.m_petsc.create(PETSc.COMM_WORLD)
            self.m_petsc.createAIJWithArrays(
                size=m_csr.shape, csr=(m_csr.indptr, m_csr.indices, m_csr.data)
            )

            self.m_petsc.assemblyBegin()
            self.m_petsc.assemblyEnd()

            self.div_coo = sparse.coo_matrix(
                (div_info[0], (div_info[1], div_info[2])),
                shape=(self.mesh.get_number_of_cells(), self.mfd.flux_dof),
            )
            ...
            self.div_petsc.createAIJWithArrays(
                size=(self.mesh.get_number_of_cells(), self.mfd.flux_dof),
                csr=(self.div_csr.indptr, self.div_csr.indices, self.div_csr.data),
            )

            self.div_petsc.setUp()

            self.div_t_coo = sparse.coo_matrix(
                (div_t_info[0], (div_t_info[1], div_t_info[2])),
                shape=(self.mfd.flux_dof, self.mesh.get_number_of_cells()),
            )
            ...
            self.div_t_petsc.createAIJWithArrays(
                size=(self.mfd.flux_dof, self.mesh.get_number_of_cells()),
                csr=(
            ...
            self.c_coo = sparse.coo_matrix(
                (c_info[0], (c_info[1], c_info[2])),
                shape=(
            ...
            self.c_petsc.createAIJWithArrays(
                size=(self.mesh.get_number_of_cells(), self.mesh.get_number_of_cells()),
                csr=(c_csr.indptr, c_csr.indices, c_csr.data),
            )
            ...
            self.schur_mat = SchurComplementMat(
                self.div_petsc, self.div_t_petsc, self.m_petsc, self.c_petsc
            )
            ...
            m_diag = 1.0 / m_diag
            m_diag = sparse.csr_matrix(
                (
                    m_diag,
                    (list(range(self.mfd.flux_dof)), list(range(self.mfd.flux_dof))),
                )
            )
            ...
            self.pc_petsc.createAIJWithArrays(
                size=(self.mesh.get_number_of_cells(), self.mesh.get_number_of_cells()),
                csr=(pc_matrix.indptr, pc_matrix.indices, pc_matrix.data),
            )
    ...
    def update_pressure(self, time_step):
        ...
        while abs(newton_residual > self.newton_threshold):
            ...
            current_c_matrix += self.ref_density_oil * (
                self.compressibility_oil * (1.0 - self.current_s_w)
            )
            ...
            self.mfd.update_m(
                self.lhs_coo.data[: self.m_x_coo_length], current_total_mobility
            )

            for cell_index, pressure_pi in zip(
                self.pressure_wells, self.pressure_wells_pi
            ):
                current_c_matrix[cell_index] += (
                    pressure_pi * 1.0 / current_total_mobility[cell_index]
                )
            ...
            f2sum3_l += self.ref_density_water * (
                1.0 + self.compressibility_water * (self.current_p_o)
            )
            f2sum3_l *= self.current_s_w
            f2sum3_l += (
                self.ref_density_oil
                * (1 + self.compressibility_oil * self.current_p_o)
                * (1.0 - self.current_s_w)
            )
            ...
            for well_index, cell_index in enumerate(self.rate_wells):
                rhs[cell_index + self.mfd.flux_dof] += -self.get_well_rate_water(
                    well_index
                )
                rhs[cell_index + self.mfd.flux_dof] += -self.get_well_rate_oil(
                    well_index
                )

            for cell_index, bhp, pressure_pi in zip(
                self.pressure_wells, self.pressure_wells_bhp, self.pressure_wells_pi
            ):
                rhs[cell_index + self.mfd.flux_dof] -= (
                    pressure_pi * bhp * 1.0 / current_total_mobility[cell_index]
                )
            ...
            if newton_residual > self.newton_threshold:
                ...
                if self.solver == 1:
                    ...
                    f1_minvp.setArray(
                        -rhs[: self.mfd.flux_dof] - self.div_t_coo.dot(y.getArray())
                    )
    ...
    def update_saturation(self, time_step):
            ...
            for (
                ...
                if boundary_orientation * self.current_u_t[flux_index] < 0:
                    ...
                    current_pressure = np.array(
                        [self.mfd.get_dirichlet_value(boundary_index)]
                    )
                    self.current_f_w[flux_index] = self.water_mobility(
                        current_saturation, current_pressure
                    )

                    self.current_f_w[flux_index] /= self.water_mobility(
                        current_saturation, current_pressure
                    ) + self.oil_mobility(current_saturation, current_pressure)
        ...
        for well_number in range(len(self.pressure_wells)):
            ...
            water_production /= (
                1.0 + self.compressibility_water * current_p_w[cell_index]
            )
            ...
            oil_production /= (
                1.0 + self.compressibility_oil * self.current_p_o[cell_index]
            )
