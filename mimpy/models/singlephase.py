...
class SinglePhase:
    ...
    def start_solving(self):
        ...
        self.mesh.output_vtk_mesh(
            self.model_name + "0",
            [self.current_pressure, self.mesh.get_cell_domain_all()],
            ["pressure", "domain"],
        )
        ...
        for time_step in range(1, self.number_of_time_steps + 1):
            ...
            for cell_index in range(self.mesh.get_number_of_cells()):
                ...
                rhs_current[self.mfd.flux_dof + cell_index] += (
                    c_entry * self.current_pressure[cell_index]
                )

                self.lhs_coo.data[self.c_start + cell_index] = c_entry

            for [index, cell_index] in enumerate(self.rate_wells):
                rhs_current[self.mfd.flux_dof + cell_index] += self.rate_wells_rate[
                    index
                ]
    ...
    def start_solving_newton(self):
        ...
        for time_step in range(1, self.number_of_time_steps + 1):
            ...
            print("\n")
            for i in range(100):
                rhs_current = np.zeros(
                    self.mfd.get_number_of_dof(), dtype=np.dtype("d")
                )
                rhs_current += self.rhs_mfd
                for cell_index in range(self.mesh.get_number_of_cells()):
                    density = -self.ref_pressure
                ...
                self.mfd.update_m(
                    self.lhs_coo.data[: self.m_x_coo_length], m_multipliers
                )
