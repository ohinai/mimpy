...
class TestCellDivide(unittest.TestCase):
    def test_write(self):
        ...
        two_cells.build_mesh(2, 3, 2, 1.0, 1.0, 1.0, lambda x, i, j, k: np.eye(3))
        ...
        assert two_cells.get_number_of_faces() == loaded_mesh.get_number_of_faces()

        assert two_cells.get_number_of_cells() == loaded_mesh.get_number_of_cells()

    def test_write_single(self):
        two_cells = hexmesh.HexMesh()
        two_cells.build_mesh(2, 2, 2, 1.0, 1.0, 1.0, lambda x, i, j, k: np.eye(3))
